export const IS_CONNECTED = 'IS_CONNECTED';
export const FETCH_USER_CHANNELS = 'FETCH_USER_CHANNELS';
export const ADD_USER_TO_CACHE = 'ADD_USER_TO_CACHE';
export const GET_USER_TOKEN_ID = 'GET_USER_TOKEN_ID';
export const LOAD_USER_AVATAR = 'LOAD_USER_AVATAR';
export const START_SEARCH_USERS = 'START_SEARCH_USERS';
export const SET_USER_TOKEN = 'SET_USER_TOKEN';
export const GET_TOKEN_FROM_LOCAL_STORAGE = 'GET_TOKEN_FROM_LOCAL_STORAGE';
export const GET_USER_FROM_LOCAL_STORAGE = 'GET_USER_FROM_LOCAL_STORAGE';
export const SET_CURRENT_USER = 'SET_CURRENT_USER';
export const CLEAR_CACHE_DATA = 'CLEAR_CACHE_DATA';
export const SIGN_OUT = 'SIGN_OUT';
export const REGISTER = 'REGISTER';
export const LOGIN = 'LOGIN';
export const REMOVE_MEMBER_FROM_CHANNEL = 'REMOVE_MEMBER_FROM_CHANNEL';
export const ADD_USER_TO_CHANNEL = 'ADD_USER_TO_CHANNEL';
export const GET_SEARCH_USERS = 'GET_SEARCH_USERS';
export const ON_CREATE_NEW_CHANNEL = 'ON_CREATE_NEW_CHANNEL';
export const GET_CURRENT_USER = 'GET_CURRENT_USER';
export const FETCH_CHANNEL_MESSAGES = 'FETCH_CHANNEL_MESSAGES';
export const GET_ACTIVE_CHANNEL = 'GET_ACTIVE_CHANNEL';
export const SET_ACTIVE_CHANNEL_ID = 'SET_ACTIVE_CHANNEL_ID';
export const ADD_MESSAGE = 'ADD_MESSAGE';
export const SET_MESSAGE = 'SET_MESSAGE';
export const GET_MESSAGES = 'GET_MESSAGES';
export const GET_MESSAGES_FROM_CHANNEL = 'GET_MESSAGES_FROM_CHANNEL';
export const GET_MEMBERS_FROM_CHANNEL = 'GET_MEMBERS_FROM_CHANNEL';
export const ADD_CHANNEL = 'ADD_CHANNEL';
export const GET_CHANNELS = 'GET_CHANNELS';
export const UPDATE_LAST_MESSAGE = 'UPDATE_LAST_MESSAGE';
export const ORDER_CHANNELS = 'ORDER_CHANNELS';
export const ORDER_CHANNELS_BY_THE_LATEST_MESSAGE = 'ORDER_CHANNELS_BY_THE_LATEST_MESSAGE';